...
pub fn contributor_agreement_0_2_0(arguments: &ArgMatches) -> String {
    let project_name = match arguments.value_of("project-name") {
        Some(name) => name,
        None => {
            eprintln!("[Error] <PROJECT_NAME> for --project-name option must be provided");
            exit(1);
        }
    };
...
12. If any part of this AGREEMENT is found to be invalid or
    ...
    will be inoperative.",
        project_name,
        git_string(GitString::Name),
        git_string(GitString::Email)
    )
}
